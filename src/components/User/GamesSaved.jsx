  ...
  return (
    <div className="w-full flex justify-center flex-col">
      <div className="hidden md:block self-center w-2/3">
        <h3 className="text-end dark:text-lightBackground mr-[-97px] mt-[80px] text-xl">
          Number of games played : {props.games.length}
        </h3>
      </div>

      <div className="w-full flex justify-center items-center flex-col pb-[30px]">
        {props.games.length > 0 &&
          props.games.map((game, index) => (
            <div
              key={index}
              className={`rounded-md p-4 w-4/5 mt-6 grid grid-cols-4 md:grid-cols-5 grid-rows-1 items-center justify-items-cente bg-opacity-10 ${getImageAndColorOfGame(game).color}`}
            >
              <img
              ...
              />
              <p className="text-center font-bold md:block hidden dark:text-lightBackground">
                {game.game}
              </p>
